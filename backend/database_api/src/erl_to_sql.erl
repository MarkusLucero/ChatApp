...
loop(Ref) ->
	...
	{insert_chat, From_Username, Chat_ID, { Timestamp, Msg}, Status} ->
	    ...
	    X = odbc:sql_query(Ref, "CREATE TABLE IF NOT EXISTS chat" ++ Chat_ID ++ "  (from_user VARCHAR(50) NOT NULL, message TEXT NOT NULL, status INT, time_stamp TIMESTAMP NOT NULL);"),

	    Y =odbc:sql_query(Ref, "INSERT INTO chat" ++ Chat_ID ++ " VALUES ('"++ From_Username ++ "', '" ++ Msg ++ "', '" ++ integer_to_list(Status) ++ "', '" ++ Timestamp ++ "');"),
