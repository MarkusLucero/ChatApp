        ...
        {login_user, Username, Magic_Token, DMs, FriendList, PID} ->
            ...
            case check_token(Username, Magic_Token) of
                true ->
                    ...
                    Members = fun(ID) -> [Tuple] = database_api:fetch_chat_members(ID), tuple_to_list(Tuple) end,
                    List_of_DMs = [{struct, [{"chatName", Chat_Name}, 
                                             {"chatID", Chat_ID},
                                             {"members", {array, Members(Chat_ID)}},
                                             {"messages", {array, [{struct, [{"message", Msg}, {"username", Src}, {"timestamp", Timestamp}]} || {Src, Msg, Timestamp} <- Messages]}}
